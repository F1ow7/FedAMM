...
from utils.defense import get_weight
...
class LocalUntargetedMaliciousUpdate(object):
    def __init__(self, args, dataset=None, idxs=None, attack=None):
        self.args = args
        self.loss_func = nn.CrossEntropyLoss()
        self.selected_clients = []
        self.ldr_train = DataLoader(DatasetSplit(
    ...
    def train(self, net):
        if self.attack == 'fla':
            return self.train_malicious_fla(net)
        elif self.attack == 'gna':
            return self.train_malicious_gna(net)
        # elif self.attack == 'random':
        #     return self.train_malicious_random(net)
    ...
    def train_malicious_fla(self, net, test_img=None, dataset_test=None, args=None):
        net.train()
        # train and update
        optimizer = torch.optim.SGD(
            net.parameters(), lr=self.args.lr, momentum=self.args.momentum)
    ...
    def train_malicious_gna(self, net, test_img=None, dataset_test=None, args=None):

        global_params = net.state_dict()
        global_weight = get_weight(global_params)
        weight_std = torch.std(global_weight)
        model_attack = global_weight + 0.05 * weight_std * torch.randn(global_weight.size()).cuda()
        model_attack = model_attack.squeeze(0)
        current_idx = 0
        for key in global_params:
            length = len(global_params[key].reshape(-1))
            global_params[key] = model_attack[current_idx : current_idx + length].reshape(global_params[key].shape)
            current_idx += length
        net.load_state_dict(global_params)

        return net.state_dict(), None
